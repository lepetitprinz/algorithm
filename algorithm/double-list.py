...
class Node:
    ...
    def __init__(self, data:Any=None, prev:Node = None, next:Node = None) -> None:
        """초기화"""
        self.data = data # 데이터
        self.prev = prev or self # 앞쪽 포인터( prev가 None -> self, None이 아니면 -> prev )
        self.next = next or self # 뒤쪽 포인터( next가 None -> self, None이 아니면 -> next )
...
class DoubleLinkedList:
    ...
    def __init__(self) ->None:
        """초기화"""
        self.head = self.current = Node()  # 더미 노드(데이터가 없는 노드)를 작성
        self.no = 0
    ...
    def print_reverse(self) -> None:
        """모든 노드를 역순으로 출력"""
        ptr = self.head.prev # 더미 노드의 앞뽁 노드
        while ptr is not self.head:
            print(ptr.data)
            ptr = ptr.prev
    ...
    def clear(self) ->None:
        """모든 노드를 삭제"""
        while not self.is_empty():  # 리스트 전체가 빌 때까지 진행
